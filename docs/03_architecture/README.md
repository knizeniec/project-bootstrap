# Architecture Documentation

Status: Active
Owner: [Role or team responsible for architecture docs]
Purpose: provide the landing page for solution design, integrations, runtime patterns, and technical decisions
Last updated: YYYY-MM-DD

## Use this area for

- solution design and system boundaries
- data flow, interfaces, and deployment patterns
- implementation patterns that support the active architecture baseline

## Common documents

- solution design document
- integrations and data-flow documentation
- interface or contract documentation
- architecture overview diagrams

## Starter templates

- [01_solution_design_TEMPLATE.md](01_solution_design_TEMPLATE.md)
- [06_interface_control_document_TEMPLATE.md](06_interface_control_document_TEMPLATE.md)

## Related documents

- [../project/adr/README.md](../project/adr/README.md) for durable architecture decisions.
- [../00_governance/README.md](../00_governance/README.md) for traceability and control ownership.
- [../INDEX.md](../INDEX.md) for full navigation.
